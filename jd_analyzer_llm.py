...
def analyze_jd_with_llm(job_description: str) -> dict:
    """
    Analyzes a job description using Ollama's llama3.2 model to extract structured requirements.
    Returns a dict with keys: job_title, domain, must_haves, nice_to_haves, experience, title_keywords.
    Handles errors gracefully by returning None if the API call fails.
    """
    url = "http://localhost:11434/api/generate"
    prompt = f"""
Extract the following from the job description below as a valid JSON object. Focus on quality terms only (no generic words like "years" or "experience" unless part of a specific requirement). Structure the output exactly like this:
{{
    "job_title": "string",
    "domain": "string",
    "must_haves": ["list", "of", "key", "skills"],
    "nice_to_haves": ["list", "of", "optional", "skills"],
    "experience": "string describing required experience level",
    "title_keywords": ["list", "of", "keywords", "from", "title"]
}}
...
"""
    ...
    try:
        ...
        result = response.json()
        # Assuming the response has a 'response' key with the JSON string
        extracted = json.loads(result.get("response", "{}"))
        # Validate structure (basic check)
        required_keys = ["job_title", "domain", "must_haves", "nice_to_haves", "experience", "title_keywords"]
